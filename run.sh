...
IMAGE_NAME="${1}"
OUTPUT_FILE="${2:-playwright-resource-usage.txt}${3}"
...
docker run --mount type=bind,src="$(pwd)/playwright.config.js",dst=/app/playwright.config.js --name "$CONTAINER_NAME" -d "$IMAGE_NAME"\
  npx playwright test
